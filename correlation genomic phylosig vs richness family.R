## Correlation Analysis of Phylogenetic Signal and Species Richness (Spearman and Pearson) for Genomic Features
...
phylo_signal_file <- "D:/Akash Ajay/insect bite force project/genome features/species richness/signal family.xlsx"
...
species_richness_file <- "D:/Akash Ajay/insect bite force project/genome features/species richness/richness family.xlsx"
...
cat("Phylogenetic signal data - Unique families:", length(unique(phylo_signal_data$Family)), "\n")
cat("Species richness data - Unique families:", length(unique(species_richness_data$Family)), "\n")

# Check for duplicate Family+Trait combinations in phylogenetic data
...
if(nrow(duplicates) > 0) {
  cat("Warning: Found", nrow(duplicates), "duplicate Family+Trait combinations:\n")
  print(duplicates %>% select(Family, Trait) %>% distinct())
...
merged_data <- merge(phylo_signal_data, species_richness_data, by = "Family", all.x = FALSE)

cat("After merging - Total observations:", nrow(merged_data), "\n")
cat("Families in merged data:", length(unique(merged_data$Family)), "\n")
...
for (trait in traits) {
  ...
  if (nrow(trait_data) < 3) {
    cat("Insufficient data for trait", trait, " (", nrow(trait_data), " families). Skipping.\n")
    correlation_results <- rbind(correlation_results, data.frame(
      Trait = trait,
      N_Families = nrow(trait_data),
  ...
  cat("Analyzing", nrow(trait_data), "data points from", length(unique(trait_data$Family)), "families\n")
...
output_file <- "D:/Akash Ajay/insect bite force project/genome features/species richness/family_corr.xlsx"
write.xlsx(correlation_results, output_file, sheetName = "Correlation_Results", rowNames = FALSE)

## Step 9: Print final summary
cat("\n===== Correlation Analysis Summary =====\n")
cat("Analyzed", length(traits), "traits across", length(unique(merged_data$Family)), "families\n")
