...
sink("phyr_log_wing_length.txt", append = TRUE, split = FALSE)
...
process_order_phyr <- function(order, predictor) {
  cat("\n=== Processing", order, "with predictor", predictor, "for Wing_L ===\n")
  ...
  required_traits <- c("Wing_L", predictor)
  ...
  if (length(available_data_traits) < 2) {
    warning_log <<- c(warning_log, paste("Error: Missing required traits (Wing_L or", predictor, ") in", order))
  ...
  if (!is.null(phyr_model)) {
    ...
    phyr_plot <- ggplot(phyr_importance, aes(x = Predictor, y = abs(Coefficient))) +
      ...
      theme_minimal() +
      labs(title = paste("Phyr Coefficient Importance for Wing Length in", order, "with", predictor), 
           x = "Predictor Trait", 
    ...
    ggsave(paste0(predictor, "_", order, "_phyr_importance_wing_length.png"), phyr_plot, width = 8, height = 6, dpi = 800)
    ggsave(paste0(predictor, "_", order, "_phyr_importance_wing_length.pdf"), phyr_plot, width = 8, height = 6, dpi = 800)
...
for (predictor in predictors) {
  cat("\n=== Processing models for predictor:", predictor, "for Wing_L ===\n")
  ...
  write.csv(phyr_results_df, paste0(predictor, "_phyr_importance_per_order_wing_length.csv"), row.names = FALSE)
  cat("Results saved to:", paste0(predictor, "_phyr_importance_per_order_wing_length.csv"), "\n")
}
...
cat("\n=== Outputs ===\n")
cat("Log saved to: phyr_log_wing_length.txt\n")
cat("Results saved to: <predictor>_phyr_importance_per_order_wing_length.csv\n")
cat("Plots saved to: <predictor>_<order>_phyr_importance_wing_length.png/pdf (800 DPI)\n")
...
for (predictor in predictors) {
  file_name <- paste0(predictor, "_phyr_importance_per_order_wing_length.csv")
...
write.csv(merged_results, "merged_phyr_importance_wing_length.csv", row.names = FALSE)
cat("Merged results saved to: merged_phyr_importance_wing_length.csv\n")